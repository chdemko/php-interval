...
if [ "$TRAVIS_REPO_SLUG" == "chdemko/php-interval" ] && [ "$TRAVIS_PHP_VERSION" == "5.4" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ]; then
  ...
  git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/chdemko/php-interval gh-pages > /dev/null
